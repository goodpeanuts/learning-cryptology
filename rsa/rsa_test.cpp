 ...
 * @LastEditTime: 2024-01-01 14:25:26
 * @FilePath: /learning-cryptology/rsa/rsa_test.cpp
...
double generateSpeedTest(int bits)
{
    ...
    while (!isPrime)
    {
        ...
        isPrime = millerRabinTest(rand_prime, 1);
    ...
    std::cout << ++ cc << " Time taken: " << diff.count() << " ms\n";
...
int generateBigPrime(int bits)
{
    ...
    while (!isPrime)
    {
        ...
        if (isPrime)
        {
            ...
            std::cout << ++cc << "#循环次数: " << count << std::endl;
        }
        else
        {
            count++;
...
int main()
{
    ...
    while (count--)
    {
       n += generateBigPrime(2048);
    }
    std::cout << "每2048位大素数生成随机数: " << n / 1000 << " 次" <<std::endl;
